...
/**
 * base class for json parsers. Provides functionality to connect to
...
class SenMLBaseParser {
 ...
 protected:
  SenMLPack
      *root;                    //the root document for which we are parsing. Used to search the up all the actuators.
  SenMLRecord *curRec;
  SenMLPack *
      curPack;                 //used while searching, to store the current pack to use. init to root pack by default. Make it protected, so child parsers can easily access it without too much fuzz
