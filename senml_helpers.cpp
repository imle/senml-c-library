    ...
    if (_streamCtx->format == SENML_RAW) {
      for (int i = 0; i < len; i++) {
        if (_streamCtx->data.blob.curPos >= _streamCtx->data.blob.length) {
          //if we reached the end of the buffer, stop rendering otherwise we overwrite some other mem which is not good.
          return;
        }
...
int readChar() {
  int res;
  if (_streamCtx->dataAsBlob) {
    //peekchar has to return -1 if there is no more data, so check for this.
    if (_streamCtx->data.blob.curPos < _streamCtx->data.blob.length) {
    ...
    //data is comming from the stream, this already returns -1 if there is no data.
    if (_streamCtx->format == SENML_RAW) {
#ifdef __MBED__
      res = _streamCtx->data.stream->getc();
#else
      //arduino stream, check if something is available, if not, we can't read anymore.
      res = _streamCtx->data.stream->available() ? _streamCtx->data.stream->read() : -1;
#endif
    } else {
      int resB;
#ifdef __MBED__
