 ...
 * gateway.
 * 
 * A SenMLPack object is able to render and parse the data to/from a json string.
 * Both rendering and parsing can be done either directly from a stream (like an UART
 ...
 * The parser and generator are able to render in the native format (strings for json)
 * and as a hex string. This is especially useful when directly working on a stream: some modems
...
class SenMLPack : public SenMLBase {
 ...
 protected:
  ...
  inline void actuate(const char *pack,
                      const char *record,
                      const void *value,
                      int valueLength,
